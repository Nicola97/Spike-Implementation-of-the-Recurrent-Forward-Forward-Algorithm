...
def overlay_y_on_x(x, y):
    # Assicurati che entrambi gli input siano su GPU
    x = x.to(device)
    y = y.to(device)
    x_ = x.clone()
    x_[:, :10] *= 0.0
...
class Layer(nn.Linear):
    def __init__(self, in_features, out_features,
        ...
        self.layer_norm = nn.LayerNorm(out_features)  # normalizza sulla dimensione out_features
        self.to(device)
    ...
    def forward(self, x):
        ...
        linear_output = torch.mm(x_direction, self.weight.T) + self.bias.unsqueeze(0)
        #normalized_output = self.layer_norm(linear_output)
        activated_output = self.relu(linear_output)
        final_output = self.dropout(activated_output)
...
class MetaLayer(torch.nn.Module):
    ...
    def train(self, x_pos, x_neg):
        ...
        #g_pos = g_pos.unsqueeze(1)  # Aggiunge una dimensione
        #g_neg = g_neg.unsqueeze(1)
        #softmax = nn.Softmax(dim=0)
        gelu = nn.GELU()
        #loss= (0.5*(g_pos+self.threshold)*(1+torch.tanh(0.7981*(g_pos-self.threshold+0.044715*(g_pos-self.threshold)**3))) - 0.5*(g_neg+self.threshold)*(1+torch.tanh(0.7981*(g_neg-self.threshold+0.044715*(g_neg-self.threshold)**3)))).mean()
        #loss = (softmax((g_pos+self.threshold) - (g_neg+self.threshold))).mean()
        loss = (gelu(-g_pos + self.threshold) + gelu(g_neg - self.threshold)).mean()
        self.opt.zero_grad()
...
for index,row in params_df.iterrows():
    ...
    for epoch in range(epochs):
        ...
        for x, y in tqdm(train_loader):
            ...
            #print_gpu_memory()  # Stampa l'uso della memoria GPU
            ...
            """for i, layer in enumerate(net.layers):
                print(f"Layer {i} is on GPU: {next(layer.parameters()).is_cuda}")"""
