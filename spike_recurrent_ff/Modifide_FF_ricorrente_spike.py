...
class MetaLayer(nn.Module):
    def __init__(self, self_size, pre_size, post_size, out_size):
        ...
        # Inizializza una lista vuota per l'epoca corrente
        self.current_epoch_losses = []
        self.loss_history = []  # Questa conterrà le loss di tutte le epoche
    ...
    def forward(self, x, mem_pre, mem_post, mem_self):
        ...
        #spk_out = 0.5* (spk_pre + spk_post) + 0.5 * spk_self

        
        return spk_out, (mem_pre_new, mem_post_new, mem_self_new)

    def train(self, x_pos, x_neg):
        # Non reinizializzare la loss_history qui
        ...
        #loss = -torch.log(torch.sigmoid(g_pos - g_neg))
        #loss = loss.mean()      
        ...
        if stampa_training:
            print(f"Loss corrente: {loss.item()}")
            print(f"Lunghezza current_epoch_losses: {len(self.current_epoch_losses)}")
        
        # Aggiungi la loss corrente alla lista dell'epoca corrente
    ...
    def end_epoch(self):
        ...
        if self.current_epoch_losses:
            epoch_loss = sum(self.current_epoch_losses) / len(self.current_epoch_losses)
            self.loss_history.append(epoch_loss)
            if stampa_training:
                print(f"Loss media dell'epoca: {epoch_loss}")
                print(f"Lunghezza loss_history: {len(self.loss_history)}")
            self.current_epoch_losses = []  # Reset per la prossima epoca
...
for index, row in params_df.iterrows():
    ...
    for i in range(epochs):
        ...
        # Chiamata end_epoch per ogni layer alla fine di ogni epoca
    ...
    for layer_id in range(3):
        loss_history = net.layers[layer_id].loss_history
        if loss_history:  # Verifica che ci siano valori da plottare
            plt.plot(loss_history, label=f'Layer {layer_id}')
            print(f"Layer {layer_id} loss history: {loss_history}")  # Debug print

    plt.xlabel('Epoche')
    plt.ylabel('Loss')
    plt.legend()
    plt.title('Loss per Layer')
    plt.grid(True)  # Aggiungi griglia per migliore leggibilità
    save_and_show_plot(fig_loss, path_directory, "Loss media per Layer.png", show=False)  # Imposta show=True

    # Calcolo loss finali
    loss_list_layer_zero = net.layers[0].loss_history
    loss_list_layer_uno = net.layers[1].loss_history
    loss_list_layer_due = net.layers[2].loss_history

    loss_layer_zero = loss_list_layer_zero[-1] if loss_list_layer_zero else 0.0
    loss_layer_uno = loss_list_layer_uno[-1] if loss_list_layer_uno else 0.0
    loss_layer_due = loss_list_layer_due[-1] if loss_list_layer_due else 0.0
